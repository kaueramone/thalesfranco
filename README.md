...
- E-mail via Resend, com proteção contra duplicidade; WhatsApp com mensagem pronta e confirmação manual no aplicativo.
- Links revogáveis e versões imutáveis do treino.
- Modo demonstração sem persistência ou disparos reais.

**Ativação:** siga [docs/CONFIGURACAO.md](docs/CONFIGURACAO.md). Os SQLs estão em `supabase/`. Supabase configurado e URL pública são necessários. E-mail exige Resend e remetente verificado; o compartilhamento manual do WhatsApp dispensa API.
